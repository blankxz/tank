...
class Rock(GameEntity):
    def __init__(self, world, image, position):
        ...
        self.name = 'rock'
        long = self.rect.size[0] + randint(-5, 5) * self.rect.size[0] * 0.05
        width = self.rect.size[1] + randint(-5, 5) * self.rect.size[1] * 0.05
        s = long * width
        global rock_id
        id = rock_id
...
class Tank(GameEntity):
    ...
    def update(self):
        self.rect.left, self.rect.top = self.location[0] - self.image.get_width() / 2, self.location[
        ...
        if dic_collide:
            ...
            for i in dic_collide:
                ...
                if not sprite.search_data:
                    ...
                    global text_1_
                    self.target += 1
                    text_1_ = 'Found new stone : Num' + str(dic_collide[i][0].datas['id'])
                else:
                    if dic_collide[i][0].datas not in sprite.search_data:
                        sprite.search_data.append(dic_collide[i][0].datas)
                print(sprite.search_data)
                # for j in rocks:
    ...
    def barrier(self, collide):
        for i in collide:
            if collide[i][0].datas in sprite.search_data:
                global text_1_
                text_1_ = 'Found stone : Num' + str(collide[i][0].datas['id'])
...
text_1_ = 'Patrolling……'
font = pygame.font.Font('./font/simkai.ttf', 40)
